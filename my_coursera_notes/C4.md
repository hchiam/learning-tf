...
```py
window_size = 20
batch_size = 128 # try increasing (or decreasing) to see if that reduces overfitting or reduces noise/instability
...
model = tf.keras.models.Sequential([
  ...
  tf.keras.layers.Conv1D( # CNN layer
      ...
      padding='causal',
      activation='relu',
      input_shape=[None, 1] # reshape to work with the later LSTM layer
  ),
  tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32, return_sequences=True)), # LSTM layer
  # tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32, return_sequences=True)),
  tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32)),
  tf.keras.layers.Dense(1),
  tf.keras.layers.Lambda(lambda x: x * 200) # scale to same range as tanh [-1,+1]
])
model.compile(
  # loss='mse',
