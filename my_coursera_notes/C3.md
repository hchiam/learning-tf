...
- note that if the validation accuracy or loss are getting worse, there may be overfitting potentially (especially with a small model and lots of OOV words in the validation set), so try tweaking the parameters or layers
