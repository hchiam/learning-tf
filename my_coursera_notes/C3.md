...
Instead of generating a word from thin air, predict the next word, based on a corpus of example sentences to have a sense of the next expected word in a sentence.
...
for line in corpus:
  token_list = tokenizer.texts_to_sequence([line])[0] # e.g. 'A quick ...' --> [4 2 ...]
...
xs = input_sequences[:, :-1]    # e.g. [4 2 66]
labels = input_sequences[:, -1] # e.g. 8
...
model.add(Bidirectional(LSTM(20))) # Bidirectional helps carry context backwards too
...
model.fit(xs, ys, epochs=500, verbose=1) # many epochs because very little data
```
...
def continued_sentence(seed_text, next_few_words):
  for _ in range(next_few_words):
    ...
    # predicted_next_token = model.predict(token_list)
